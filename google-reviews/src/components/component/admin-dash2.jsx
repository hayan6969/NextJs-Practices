...
* @see https://v0.dev/t/g9gJJ1I12UG
...
export function AdminDash2() {
    ...
    setOrders((prevOrders) => {
      return prevOrders.map((order) => {
        if (order.id === orderId && (order.status === "Partially Fulfilled" || order.status === "Fulfilled")) {
          setCompletedOrders((prevCompletedOrders) => [
            ...prevCompletedOrders,
            { ...order, finalCost: order.finalCost },
          ])
  ...
  useEffect(() => {
    if (completedOrders.length > 0) {
      ...
      toast.success(
        `Order ${lastCompletedOrder.id} with a final cost of ${lastCompletedOrder.finalCost} € has been completed.`
      )
    }
  }, [completedOrders])
  return (
                  ...
                  {(order.status === "Partially Fulfilled" || order.status === "Fulfilled") && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        console.log("Order with the following details completed:", order.id, order.reviews, order.totalCost)
                      }}
                      className="mr-2"
